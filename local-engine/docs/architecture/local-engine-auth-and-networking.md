...
## Migration checklist (schema)

- `User`: drop `githubId`, `githubUsername`; rename `githubToken` →
  `personalAccessToken` (same encrypted-TEXT shape).
- Drop `GithubInstallation` model entirely; drop its relation off `User`.
- `Project`: drop `installationId` + `installation` relation; keep
  `repositoryId` (webhook lookup key) and `isPrivate`.
- This is a real Prisma migration (`prisma migrate dev` locally, `migrate
  deploy` on the VPS via the existing `entrypoint.sh` step) — not just an
  `.env` change. Existing installs need to run it once.

## What's actually done vs. deferred

Implemented in this pass (verified by reading every touch point, not
guessed): `schema.prisma`, `lib/env.ts`, the `auth/*` module, deletion of
`lib/github-app.ts` and `integrations/github-app-install.*`, the new
`lib/git-credentials.ts`, `workers/build.worker.ts`, `deployments/deployment-engine.ts`'s
`BuildJob` shape, `webhooks/github-webhook.*`, `projects/project.service.ts`
+ `project.types.ts`, `app.ts` route mounting, `nginx/templates/*.template`,
`docker-compose.yml`, `scripts/lib/issue-certificate.sh`, `.env.example`.

**Not done in this pass — real remaining work, not stubbed or faked:**

1. `integrations/github-repo.*` — needs the installation-list endpoint
   removed and `listReposHandler` switched to `GET /user/repos` against
   the stored PAT. Small, mechanical, not done here for lack of remaining
   scope in this pass.
2. Frontend: remove `components/landing-page/LandingPage.tsx` and the
   root route it's mounted on; add a `/setup` page (single form: name,
   email, password → `POST /api/auth/setup`, only ever shown/usable once);
   strip the GitHub OAuth buttons off `/login`; remove `/register`,
   `/verify-email`, `/forgot-password`, `/reset-password`,
   `/auth/callback`; add a "Git Personal Access Token" field to
   `app/dashboard/account`.
3. `install.sh` — update its interactive prompts to ask for a PAT instead
   of GitHub App credentials, and to request only `*.${DOMAIN}` from
   `issue-certificate.sh`, matching the script's own updated signature.
4. `scripts/reset-admin-password.ts` — small standalone script,
   referenced above, not yet written.

Each of these is scoped precisely enough to pick up independently; none of
them block the other — the backend changes above are already internally
consistent and are the part with actual security weight.
